...
\begin{itemize}
	\item \textbf{Function:}
	      \begin{itemize}
		      ...
		      \item It is organized as a \(32 \times 8\)-bit memory, where the address width is 5 bits and each memory location stores 8 bits of data.
		      \item The memory uses a single bidirectional data bus, so read and write operations cannot occur at the same time.
		      \item The \texttt{rd} and \texttt{wr} control signals are used to enable memory read and memory write operations, respectively.
		      \item All memory operations are synchronized with the positive edge of the \texttt{clk} signal.
	      \end{itemize}
