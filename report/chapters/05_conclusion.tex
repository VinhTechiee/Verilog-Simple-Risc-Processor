...
Overall, the project successfully achieved the main requirements of designing a simple RISC processor using Verilog HDL. The processor supports the required 3-bit opcode and 5-bit operand instruction format, implements all required functional blocks, and executes the eight specified instructions. Each module was verified using an individual testbench, and the complete CPU was validated through top-level simulation. Although the design is simple, it demonstrates the essential operation of a processor, including instruction fetching, decoding, operand access, ALU execution, memory access, and program control.

\subsection{Difficulties Encountered}

Several difficulties were encountered during the implementation and testing of the processor. The first challenge was controlling the timing of internal signals across different execution states. Since each instruction is completed through multiple clock cycles, control signals for memory access, instruction loading, accumulator loading, program counter updating, and memory writing had to be activated in the correct order. Any incorrect control signal could cause the CPU to fetch the wrong instruction, update the wrong register, or access memory at the wrong time.

Another challenge was working with the shared bidirectional data bus. The same bus is used for both reading data from memory and writing data back to memory, so the design had to ensure that only one component drives the bus at a time. This was especially important for the store instruction, where the accumulator value must be placed on the data bus only during the correct write phase.

The program counter behavior also required careful verification. For normal instructions, the program counter increments sequentially. However, branch-related instructions change this behavior: the skip instruction may require an additional increment when the zero condition is satisfied, while the jump instruction loads a new address directly into the program counter. These cases were tested carefully because small timing errors could change the execution flow of the whole program.

The final difficulty was integrating all modules into one complete CPU. Although each block passed its individual testbench, the full processor introduced additional timing interactions between the controller, memory, instruction register, accumulator, ALU, and program counter. These issues were resolved by checking simulation waveforms and comparing the internal signal transitions with the expected instruction execution sequence.
...
\begin{table}[H]
	...
	\begin{tabular}{|c|l|c|p{7.5cm}|}
		...
		\hline
		2            & Lê Hiển Vinh         & 2453422             & Implemented the Program Counter and top-level CPU modules.
		\newline Developed expected output files for testbenches, and performed simulation verification for the system.                                                                                                          \\
		\hline
